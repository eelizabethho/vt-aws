...
const classesOverlap = (c1, c2) => {
  ...
  return timeToMinutes(c1.startTime) < timeToMinutes(c2.endTime) &&
         timeToMinutes(c2.startTime) < timeToMinutes(c1.endTime);
};
...
const calculateMetrics = (schedule) => {
  ...
  return {
    totalGap,
    totalDistance,
    totalWalkTime: distanceToWalkTime(totalDistance),
    totalCredits: schedule.reduce((s, c) => s + (c.credits || 3), 0)
  };
};

// ─────────────────────────────────────────────
// Optimizer — takes courses db as a parameter
// ─────────────────────────────────────────────
const findBestSchedule = (courseKeys, db) => {
...
useEffect(() => {
  const subjects = ['CS', 'MATH', 'PHYS', 'ECE', 'ENGL', 'STAT', 'CHEM', 'BIOL', 'ECON', 'HIST'];
  const transformed = {};

  Promise.all(
    subjects.map(subject =>
      fetch(`http://localhost:5000/schedule?subject=${subject}`, { signal: AbortSignal.timeout(60000) })
        .then(res => res.json())
        .then(data => {
          if (!data.error) {
            data.forEach(course => {
              const key = `${subject} ${course.code}`;
              if (!transformed[key]) {
                transformed[key] = {
                  title: course.name,
              ...
              course.schedule.forEach(slot => {
                const dayInitials = { 'Monday': 'M', 'Tuesday': 'T', 'Wednesday': 'W', 'Thursday': 'R', 'Friday': 'F' };
                transformed[key].sections.push({
                  crn: course.crn,
                  location: slot.location || 'Torgersen Hall',
                  startTime: convertTime(slot.start),
                  endTime: convertTime(slot.end),
                  days: dayInitials[slot.day] || 'M',
        ...
        .catch(err => console.log(`Failed to fetch ${subject}:`, err))
    )
  ).then(() => {
  ...
  const updateMap = useCallback(() => {
    ...
    sorted.forEach((cls, idx) => {
      ...
      marker.bindPopup(`<div style="font-size:13px;padding:4px 2px;">
        <div style="font-weight:700;font-size:15px;color:#E87722;margin-bottom:4px;">${cls.courseKey}</div>
        <div style="color:#ccc;margin-bottom:6px;">${cls.title}</div>
        <div style="color:#aaa;font-size:12px;">🕐 ${formatTime(cls.startTime)} – ${formatTime(cls.endTime)}</div>
        <div style="color:#aaa;font-size:12px;margin-top:2px;">📍 ${cls.location} · 📅 ${formatDays(cls.days)}</div>
        <div style="color:#888;font-size:11px;margin-top:4px;">👤 ${cls.instructor}</div>
      </div>`);
  ...
  const toggleCourse = (key) => {
    setError('');
    setSelectedCourses(prev => prev.includes(key) ? prev.filter(k => k !== key) : [...prev, key]);
  };
  ...
  const handleOptimize = () => {
    ...
    setOptimizing(true);
    setTimeout(() => {
      const best = findBestSchedule(selectedCourses, courses); // ← pass courses state
  ...
  // ← uses courses state instead of CLASS_DATABASE
            ...
            <div className="bg-gray-800/40 backdrop-blur-sm rounded-2xl border border-gray-700/40 overflow-hidden">
              ...
              {showCatalog && (
                <div className="border-t border-gray-700/30 max-h-72 overflow-y-auto">
                  {/* Loading state */}
                  {loading && (
                    <div className="px-6 py-8 text-center text-gray-400 text-sm flex items-center justify-center gap-2">
                      <div className="w-4 h-4 border-2 border-gray-600 border-t-[#E87722] rounded-full animate-spin" />
                      Loading courses from VT timetable...
                    </div>
                  )}

                  {/* Course list */}
                  {!loading && filteredCourses.map(([key, course]) => {
                  ...
                  {!loading && filteredCourses.length === 0 && (
                    <div className="px-6 py-8 text-center text-gray-500 text-sm">No courses match your search.</div>
                  )}
                </div>
              )}
              ...
              <div className="p-6 pt-4 border-t border-gray-700/30">
                <button
                  ...
                  disabled={optimizing || selectedCourses.length === 0 || loading}
                  ...
                  {optimizing
                    ? (<><div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />Finding Best Schedule...</>)
                    : (<><Zap className="w-5 h-5" />Optimize Schedule ({selectedCourses.length} course{selectedCourses.length !== 1 ? 's' : ''})</>)
                  }
                </button>
                {error && (
                  <div className="mt-3 p-3 bg-red-900/30 border border-red-800/50 rounded-xl text-red-300 flex items-start gap-2 text-sm">
                          ...
                          <div key={cls.crn}>
                            <div className="flex items-center gap-3">
                              <div className="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold"
                                style={{ backgroundColor: LOCATIONS[cls.location]?.color || '#E87722', color: '#fff' }}>{idx + 1}</div>
                              <div>
            ...
            {schedule.length === 0 && (
              <div className="bg-gray-800/20 rounded-2xl border border-dashed border-gray-700/40 p-12 text-center">
                ...
                <p className="text-sm text-gray-500 max-w-sm mx-auto">
                  Select your courses above and click <span className="text-[#E87722] font-medium">Optimize Schedule</span> to find the best combination with minimal walking.
                </p>
              </div>
            )}
          </div>
